...
/**
 * Vulkan 1.3 terrain-buffer skeleton for the Minecraft 26.3 module.
 */
public final class IntelVulkanBackend implements AutoCloseable {
    ...
    private long createBuffer(MemoryStack stack, long size, int usage) {
        ...
        checkResult(VK13.vkCreateBuffer(device, createInfo, null, handle), "vkCreateBuffer");
        return handle.get(0);
    }

    private long allocateAndBindMemory(MemoryStack stack, long buffer, int memoryTypeIndex) {
        ...
        LongBuffer handle = stack.mallocLong(1);
        checkResult(VK13.vkAllocateMemory(device, allocateInfo, null, handle), "vkAllocateMemory");
        checkResult(
